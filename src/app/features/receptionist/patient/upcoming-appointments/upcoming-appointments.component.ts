...
export class UpcomingAppointmentsComponent {
  ...
  async resheduleUpcomingAppointment(index: number, doctorId: string): Promise<void> {
    try {
      ...
      await firstValueFrom(
        this.patientService.resheduleUpcomingAppointment(appointment.appointmentId)
      );
  ...
  async fetchUpcomingAppointments(): Promise<void> {
    try {
      const response = await firstValueFrom(
        this.patientService.getUpcomingAppointments(this.currentPage - 1, this.itemsPerPage)
      );
  ...
  async cancelUpcomingAppointment(index: number): Promise<void> {
    try {
      ...
      await firstValueFrom(this.patientService.cancelUpcomingAppointment(appointmentId));
